...
import static org.openzen.zenscript.codemodel.type.member.BuiltinID.ARRAY_CONSTRUCTOR_PROJECTED_INDEXED;
...
public class JavaSourceExpressionFormatter implements ExpressionVisitor<ExpressionString> {
	...
	public ExpressionString visitCompare(CompareExpression expression) {
		if (expression.operator.builtin != null)
			return visitBuiltinCompare(expression, expression.operator.builtin);
	...
	public ExpressionString visitCall(CallExpression expression) {
		if (expression.member.builtin != null) {
			return visitBuiltinCall(expression, expression.member.builtin);
		} else {
			JavaSourceMethod method = expression.member.getTag(JavaSourceMethod.class);
			if (method == null)
				throw new IllegalStateException("No source method tag for " + expression.member.definition.name + "::" + expression.member.name + "!");
			...
			switch (method.kind) {
				case EXPANSION: {
					...
					FormattingUtils.formatExpansionCall(output, target, scope, expression.target.accept(this), expression.arguments);
				...
				case INSTANCE: {
					...
					output.append('.');
					output.append(method.name);
					FormattingUtils.formatCall(output, target, scope, expression.arguments);
					return new ExpressionString(output.toString(), JavaOperator.CALL);
				}
				default:
	...
	public ExpressionString visitCallStatic(CallStaticExpression expression) {
		if (expression.member.builtin != null)
			return visitBuiltinCallStatic(expression, expression.member.builtin);
	...
	public ExpressionString visitCast(CastExpression expression) {
		...
		if (!expression.isImplicit) {
			result.append("(");
			result.append(scope.type(expression.member.toType));
			result.append(")");
		}
	...
	public ExpressionString visitGetField(GetFieldExpression expression) {
		StringBuilder result = new StringBuilder();
		if (!(expression.target instanceof ThisExpression && !scope.hasLocalVariable(expression.field.name))) {
	...
	public ExpressionString visitGetter(GetterExpression expression) {
		if (expression.getter.builtin != null)
			return visitBuiltinGetter(expression, expression.getter.builtin);
	...
	public ExpressionString visitNew(NewExpression expression) {
		if (expression.constructor.builtin != null)
	...
	public ExpressionString visitPostCall(PostCallExpression expression) {
		return unaryPostfix(expression.target, expression.member.operator == OperatorType.INCREMENT ? JavaOperator.INCREMENT : JavaOperator.DECREMENT);
	}
	...
	public ExpressionString visitSetStaticField(SetStaticFieldExpression expression) {
		return new ExpressionString(
				scope.type(expression.type) + "." + expression.field.name + " = " + expression.value.accept(this).value,
				JavaOperator.ASSIGN);
	}
	...
	public ExpressionString visitStaticSetter(StaticSetterExpression expression) {
		return new ExpressionString(
				scope.type(expression.type) + "." + expression.setter.name + " = " + expression.setter.name,
				JavaOperator.ASSIGN);
	}
	...
	private Expression duplicable(Expression expression) {
		...
		VarStatement temp = new VarStatement(expression.position, scope.createTempVariable(), expression.type, expression, true);
	...
	private ExpressionString visitBuiltinCall(CallExpression call, BuiltinID builtin) {
		switch (builtin) {
			case BOOL_NOT: return unaryPrefix(call, JavaOperator.NOT);
			case BOOL_AND: return binary(call, JavaOperator.AND);
			case BOOL_OR: return binary(call, JavaOperator.OR);
			case BOOL_XOR: return binary(call, JavaOperator.XOR);
			case BOOL_EQUALS: return binary(call, JavaOperator.EQUALS);
			case BOOL_NOTEQUALS: return binary(call, JavaOperator.NOTEQUALS);
			case BYTE_NOT: return unaryPrefix(call, JavaOperator.INVERT);
			...
			case SBYTE_SUB_SBYTE: return binary(call, JavaOperator.SUB);
			case SBYTE_MUL_SBYTE: return binary(call, JavaOperator.MUL);
			case SBYTE_DIV_SBYTE: return binary(call, JavaOperator.DIV);
			case SBYTE_MOD_SBYTE: return binary(call, JavaOperator.MOD);
			...
			case SHORT_SUB_SHORT: return binary(call, JavaOperator.SUB);
			case SHORT_MUL_SHORT: return binary(call, JavaOperator.MUL);
			case SHORT_DIV_SHORT: return binary(call, JavaOperator.DIV);
			case SHORT_MOD_SHORT: return binary(call, JavaOperator.MOD);
			...
			case INT_SUB_INT: return binary(call, JavaOperator.SUB);
			case INT_MUL_INT: return binary(call, JavaOperator.MUL);
			case INT_DIV_INT: return binary(call, JavaOperator.DIV);
			case INT_MOD_INT: return binary(call, JavaOperator.MOD);
			case INT_AND_INT: return binary(call, JavaOperator.AND);
			case INT_OR_INT: return binary(call, JavaOperator.OR);
			case INT_XOR_INT: return binary(call, JavaOperator.XOR);
			case INT_SHL: return binary(call, JavaOperator.SHL);
			case INT_SHR: return binary(call, JavaOperator.SHR);
			case INT_USHR: return binary(call, JavaOperator.USHR);
			case INT_COUNT_LOW_ZEROES: return callAsStatic("Integer.numberOfTrailingZeros", call);
			case INT_COUNT_HIGH_ZEROES: return callAsStatic("Integer.numberOfLeadingZeros", call);
			case INT_COUNT_LOW_ONES: return new ExpressionString("Integer.numberOfTrailingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case INT_COUNT_HIGH_ONES: return new ExpressionString("Integer.numberOfLeadingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case UINT_NOT: return unaryPrefix(call, JavaOperator.INVERT);
			case UINT_ADD_UINT: return binary(call, JavaOperator.ADD);
			case UINT_SUB_UINT: return binary(call, JavaOperator.SUB);
			case UINT_MUL_UINT: return binary(call, JavaOperator.MUL);
			case UINT_DIV_UINT: return callAsStatic("Integer.divideUnsigned", call);
			case UINT_MOD_UINT: return callAsStatic("Integer.remainderUnsigned", call);
			case UINT_AND_UINT: return binary(call, JavaOperator.AND);
			case UINT_OR_UINT: return binary(call, JavaOperator.OR);
			case UINT_XOR_UINT: return binary(call, JavaOperator.XOR);
			case UINT_SHL: return binary(call, JavaOperator.SHL);
			case UINT_SHR: return binary(call, JavaOperator.USHR);
			case UINT_COUNT_LOW_ZEROES: return callAsStatic("Integer.numberOfTrailingZeros", call);
			case UINT_COUNT_HIGH_ZEROES: return callAsStatic("Integer.numberOfLeadingZeros", call);
			case UINT_COUNT_LOW_ONES: return new ExpressionString("Integer.numberOfTrailingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case UINT_COUNT_HIGH_ONES: return new ExpressionString("Integer.numberOfLeadingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case LONG_NOT: return unaryPrefix(call, JavaOperator.INVERT);
			case LONG_NEG: return unaryPrefix(call, JavaOperator.NEG);
			case LONG_ADD_LONG: return binary(call, JavaOperator.ADD);
			case LONG_SUB_LONG: return binary(call, JavaOperator.SUB);
			case LONG_MUL_LONG: return binary(call, JavaOperator.MUL);
			case LONG_DIV_LONG: return binary(call, JavaOperator.DIV);
			case LONG_MOD_LONG: return binary(call, JavaOperator.MOD);
			case LONG_AND_LONG: return binary(call, JavaOperator.AND);
			case LONG_OR_LONG: return binary(call, JavaOperator.OR);
			case LONG_XOR_LONG: return binary(call, JavaOperator.XOR);
			case LONG_SHL: return binary(call, JavaOperator.SHL);
			case LONG_SHR: return binary(call, JavaOperator.SHR);
			case LONG_USHR: return binary(call, JavaOperator.USHR);
			case LONG_COUNT_LOW_ZEROES: return callAsStatic("Long.numberOfTrailingZeros", call);
			case LONG_COUNT_HIGH_ZEROES: return callAsStatic("Long.numberOfLeadingZeros", call);
			case LONG_COUNT_LOW_ONES: return new ExpressionString("Long.numberOfTrailingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case LONG_COUNT_HIGH_ONES: return new ExpressionString("Long.numberOfLeadingZeros(~" + call.target.accept(this).value + ")", JavaOperator.CALL);
			case ULONG_NOT: return unaryPrefix(call, JavaOperator.INVERT);
			case ULONG_ADD_ULONG: return binary(call, JavaOperator.ADD);
			case ULONG_SUB_ULONG: return binary(call, JavaOperator.SUB);
			case ULONG_MUL_ULONG: return binary(call, JavaOperator.MUL);
			case ULONG_DIV_ULONG: return callAsStatic("Long.divideUnsigned", call);
			case ULONG_MOD_ULONG: return callAsStatic("Long.remainderUnsigned", call);
			case ULONG_AND_ULONG: return binary(call, JavaOperator.AND);
			case ULONG_OR_ULONG: return binary(call, JavaOperator.OR);
			case ULONG_XOR_ULONG: return binary(call, JavaOperator.XOR);
			case ULONG_SHL: return binary(call, JavaOperator.SHL);
			case ULONG_SHR: return binary(call, JavaOperator.USHR);
			case ULONG_COUNT_LOW_ZEROES: return callAsStatic("Long.numberOfTrailingZeros", call);
			...
			case FLOAT_MUL_FLOAT: return binary(call, JavaOperator.MUL);
			case FLOAT_DIV_FLOAT: return binary(call, JavaOperator.DIV);
			case FLOAT_MOD_FLOAT: return binary(call, JavaOperator.MOD);
			case DOUBLE_NEG: return unaryPrefix(call, JavaOperator.NEG);
			case DOUBLE_ADD_DOUBLE: return binary(call, JavaOperator.ADD);
			case DOUBLE_SUB_DOUBLE: return binary(call, JavaOperator.SUB);
			case DOUBLE_MUL_DOUBLE: return binary(call, JavaOperator.MUL);
			case DOUBLE_DIV_DOUBLE: return binary(call, JavaOperator.DIV);
			case DOUBLE_MOD_DOUBLE: return binary(call, JavaOperator.MOD);
			case CHAR_ADD_INT: return cast(binary(call, JavaOperator.ADD), "char");
			case CHAR_SUB_INT: return cast(binary(call, JavaOperator.SUB), "char");
			case CHAR_SUB_CHAR: return binary(call, JavaOperator.SUB);
			case CHAR_REMOVE_DIACRITICS: throw new UnsupportedOperationException("Not yet supported!");
			case CHAR_TO_LOWER_CASE: return callAsStatic("Character.toLowerCase", call);
			case CHAR_TO_UPPER_CASE: return callAsStatic("Character.toUpperCase", call);
			case STRING_ADD_STRING: return binary(call, JavaOperator.ADD);
			case STRING_INDEXGET: return call("charAt", call);
			...
			case STRING_TRIM: return call("trim", call);
			case STRING_TO_LOWER_CASE: return call("toLowerCase", call);
			case STRING_TO_UPPER_CASE: return call("toUpperCase", call);
			case ASSOC_INDEXGET: return call("get", call);
			case ASSOC_INDEXSET: return call("put", call);
			case ASSOC_CONTAINS: return call("containsKey", call);
			case ASSOC_GETORDEFAULT: return call("get", call);
			case ASSOC_EQUALS: throw new UnsupportedOperationException("Not yet supported!");
			case ASSOC_NOTEQUALS: throw new UnsupportedOperationException("Not yet supported!");
			case ASSOC_SAME: return binary(call, JavaOperator.EQUALS);
			case ASSOC_NOTSAME: return binary(call, JavaOperator.NOTEQUALS);
			case GENERICMAP_GETOPTIONAL: return call("get", call);
			...
			case FUNCTION_SAME: return binary(call, JavaOperator.EQUALS);
	...
	private ExpressionString visitBuiltinCallStatic(CallStaticExpression call, BuiltinID builtin) {
		switch (builtin) {
			case BOOL_PARSE: return callStatic("Boolean.parseBoolean", call);
			case BYTE_PARSE: return callStatic("Integer.parseInt", call);
			case BYTE_PARSE_WITH_BASE: return callStatic("Integer.parseInt", call);
			case SBYTE_PARSE: return callStatic("Byte.parseByte", call);
			case SBYTE_PARSE_WITH_BASE: return callStatic("Byte.parseByte", call);
			case SHORT_PARSE: return callStatic("Short.parseShort", call);
			...
			case LONG_PARSE_WITH_BASE: return callStatic("Long.parseLong", call);
			case ULONG_PARSE: return callStatic("Long.parseUnsignedLong", call);
			case ULONG_PARSE_WITH_BASE: return callStatic("Long.parseUnsignedLong", call);
			case FLOAT_FROM_BITS: return callStatic("Float.intBitsToFloat", call);
			case FLOAT_PARSE: return callStatic("Float.parseFloat", call);
			case DOUBLE_FROM_BITS: return callStatic("Double.longBitsToDouble", call);
			case DOUBLE_PARSE: return callStatic("Double.parseDouble", call);
		}
	...
	private ExpressionString visitBuiltinCompare(CompareExpression call, BuiltinID builtin) {
		switch (builtin) {
			case BYTE_COMPARE: return compare(
					call.left.accept(this).unaryPostfix(JavaOperator.AND_FF),
					call.right.accept(this).unaryPostfix(JavaOperator.AND_FF),
					call.comparison);
			case SBYTE_COMPARE: return compare(call.left, call.right, call.comparison);
			case SHORT_COMPARE: return compare(call.left, call.right, call.comparison);
			case USHORT_COMPARE: return compare(
			...
			case STRING_COMPARE: return compare(call("compareTo", call.left, call.right), call.comparison);
			case ENUM_COMPARE: return compare(call("compareTo", call.left, call.right), call.comparison);
		}
	...
	private ExpressionString visitBuiltinStaticGetter(StaticGetterExpression call, BuiltinID builtin) {
		switch (builtin) {
			...
			case SBYTE_GET_MIN_VALUE: return new ExpressionString("Byte.MIN_VALUE", JavaOperator.MEMBER);
			case SBYTE_GET_MAX_VALUE: return new ExpressionString("Byte.MAX_VALUE", JavaOperator.MEMBER);
			case SHORT_GET_MIN_VALUE: return new ExpressionString("Short.MIN_VALUE", JavaOperator.MEMBER);
			case SHORT_GET_MAX_VALUE: return new ExpressionString("Short.MAX_VALUE", JavaOperator.MEMBER);
			case USHORT_GET_MIN_VALUE: return new ExpressionString("0", JavaOperator.PRIMARY);
			case USHORT_GET_MAX_VALUE: return new ExpressionString("65535", JavaOperator.PRIMARY);
			case INT_GET_MIN_VALUE: return new ExpressionString("Integer.MIN_VALUE", JavaOperator.MEMBER);
			case INT_GET_MAX_VALUE: return new ExpressionString("Integer.MAX_VALUE", JavaOperator.MEMBER);
			case UINT_GET_MIN_VALUE: return new ExpressionString("0", JavaOperator.PRIMARY);
			case UINT_GET_MAX_VALUE: return new ExpressionString("-1", JavaOperator.PRIMARY);
			case LONG_GET_MIN_VALUE: return new ExpressionString("Long.MIN_VALUE", JavaOperator.MEMBER);
			...
			case DOUBLE_GET_MAX_VALUE: return new ExpressionString("Double.MAX_VALUE", JavaOperator.MEMBER);
	...
	private ExpressionString visitBuiltinCast(CastExpression cast) {
		switch (cast.member.builtin) {
			...
			case BYTE_TO_DOUBLE: cast(castPostfix(cast, JavaOperator.AND_FF), "double");
			case BYTE_TO_CHAR: return castImplicit(cast, "char");
			case BYTE_TO_STRING: return callStatic("Integer.toString", castPostfix(cast, JavaOperator.AND_FF));
			case SBYTE_TO_BYTE: return castImplicit(cast, "int");
			case SBYTE_TO_SHORT: return castImplicit(cast, "short");
			...
			case SBYTE_TO_DOUBLE: return castImplicit(cast, "double");
			case SBYTE_TO_CHAR: return castImplicit(cast, "char");
			case SBYTE_TO_STRING: return callStatic("Byte.toString", cast.target);
			case SHORT_TO_BYTE: return castImplicit(cast, "int");
			case SHORT_TO_SBYTE: return castImplicit(cast, "byte");
			...
			case SHORT_TO_DOUBLE: return castImplicit(cast, "double");
			case SHORT_TO_CHAR: return castImplicit(cast, "char");
			case SHORT_TO_STRING: return callStatic("Short.toString", cast.target);
			case USHORT_TO_BYTE: return castImplicit(cast, "int");
			case USHORT_TO_SBYTE: return cast(cast, "byte");
			case USHORT_TO_SHORT: return cast(cast, "short");
			case USHORT_TO_INT: return castPostfix(cast, JavaOperator.AND_FFFF);
			...
			case USHORT_TO_STRING: return callStatic("Integer.toString", castPostfix(cast, JavaOperator.AND_FFFF));
			case INT_TO_BYTE: return cast.target.accept(this);
			case INT_TO_SBYTE: return cast(cast, "byte");
			case INT_TO_SHORT: return cast(cast, "short");
			case INT_TO_USHORT: return cast.target.accept(this);
			case INT_TO_UINT: return cast.target.accept(this);
			case INT_TO_LONG: return castImplicit(cast, "long");
			case INT_TO_ULONG: return castImplicit(cast, "long");
			case INT_TO_FLOAT: return castImplicit(cast, "float");
			...
			case UINT_TO_USHORT: return cast.target.accept(this);
			...
			case UINT_TO_STRING: return callStatic("Integer.toUnsignedString", cast.target);
			case LONG_TO_BYTE: return cast(cast, "int");
			case LONG_TO_SBYTE: return cast(cast, "byte");
			case LONG_TO_SHORT: return cast(cast, "short");
			case LONG_TO_USHORT: return cast(cast, "int");
			...
			case LONG_TO_STRING: return callStatic("Long.toString", cast.target);
			case ULONG_TO_BYTE: return cast(cast, "int");
			case ULONG_TO_SBYTE: return cast(cast, "byte");
			case ULONG_TO_SHORT: return cast(cast, "short");
			case ULONG_TO_USHORT: return cast(cast, "int");
			...
			case FLOAT_TO_USHORT: return cast(cast, "int");
			case FLOAT_TO_INT: return cast(cast, "int");
			...
			case DOUBLE_TO_SBYTE: return cast(cast, "byte");
			case DOUBLE_TO_SHORT: return cast(cast, "short");
			case DOUBLE_TO_USHORT: return cast(cast, "int");
			case DOUBLE_TO_INT: return cast(cast, "int");
			...
			case CHAR_TO_SBYTE: return cast(cast, "byte");
			case CHAR_TO_SHORT: return cast(cast, "short");
			case CHAR_TO_USHORT: return castImplicit(cast, "int");
			case CHAR_TO_INT: return castImplicit(cast, "int");
		...
		throw new UnsupportedOperationException("Unknown builtin cast: " + cast.member.builtin);
	}
	
	private ExpressionString visitBuiltinConstructor(NewExpression expression) {
		switch (expression.constructor.builtin) {
		...
		throw new UnsupportedOperationException("Unknown builtin constructor: " + expression.constructor.builtin);
	}
}
