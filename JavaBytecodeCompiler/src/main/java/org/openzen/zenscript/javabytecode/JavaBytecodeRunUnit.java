...
public class JavaBytecodeRunUnit {
	...
	private void writeScripts() {
		...
		for (int i = 0; i < scriptParameters.size(); i++) {
			descriptorBuilder.append(scriptParameterInfo.get(i).typeDescriptor);
		}
		descriptorBuilder.append(")V");

		JavaNativeMethod runMethod = JavaNativeMethod.getStatic(new JavaClass("script", "Scripts", JavaClass.Kind.CLASS), "run", descriptorBuilder.toString(), descriptorBuilder.toString(), Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC);
		JavaCompilingMethod runMethodCompiling = new JavaCompilingMethod(scriptsClass, runMethod);
		final JavaWriter runWriter = new JavaWriter(logger, CodePosition.GENERATED, scriptsClassWriter, runMethodCompiling, null, null);
		runWriter.start();
