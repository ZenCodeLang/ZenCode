...
public class SetterMember extends FunctionalMember {
	...
	public SetterMember(
			...
			String name,
			TypeID type) {
		super(position, definition, modifiers, modifiers.isStatic() ? MethodID.staticSetter(name) : MethodID.setter(name), new FunctionHeader(BasicTypeID.VOID, new FunctionParameter(type, "$")));
